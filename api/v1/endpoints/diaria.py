...
async def calcular_diarias(
    ...
    if not trechos:
        raise HTTPException(
            status_code=400, detail="Lista de trechos está vazia."
        )
    ...
    for trecho in trechos:
        ...
        if not validar_hora(trecho.hr_saida) or not validar_hora(trecho.hr_retorno):
            raise HTTPException(
                status_code=400, detail=f"Formato de hora inválido em um dos trechos: {trecho}"
            )
    ...
    match tipo_sd:
        ...
        case "complementação":
            ...
            if not validar_codigo_sd(codigo_sd):
                raise HTTPException(
                    status_code=400, detail="Formato de 'codigo_sd' inválido. Deve ser 00000/ano."
                )
        ...
        case _:
            raise HTTPException(
                status_code=400, detail="Tipo de solicitação inválido."
            )
...
async def gerar_sd(
    ...
    if tipo_sd not in ['solicitação', 'complementação', 'devolução']:
        raise HTTPException(
            status_code=400, detail="Tipo de solicitação inválido. Use 'solicitação', 'complementação' ou 'devolução'.")
    ...
    if tipo_sd == 'solicitação':
        ...
        if not codigo_sd:
            raise HTTPException(
                status_code=400, detail="O parâmetro 'codigo_sd' é obrigatório para tipos 'complementação' e 'devolução'.")
        if not validar_codigo_sd(codigo_sd):
            raise HTTPException(
                status_code=400, detail="Formato de 'codigo_sd' inválido. Deve ser 00000/ano."
            )
